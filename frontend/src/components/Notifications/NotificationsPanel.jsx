      ...
      <div className={`notification-item ${!notification.read ? 'unread' : ''}`}>
        ...
        {post && (post.image || post.imageUrl || post.videoUrl || post.mediaType === 'video') && (type === 'like' || type === 'comment') && (
          <Link to={linkTo} onClick={(e) => { e.stopPropagation(); if (!notification.read) markAsRead(notification._id); onClose(); navigate(linkTo); }}>
            {(() => {
              // Выбираем приоритетный источник превью
              const previewUrl = 
                post.thumbnailUrl || 
                post.imageUrl || 
                post.image || 
                (post.youtubeData && post.youtubeData.thumbnailUrl);

              console.log('🔍 Notification Post Preview Debug:', {
                postId: post._id,
                type: type,
              ...
              return previewUrl ? (
                <img 
                  ...
                  onError={(e) => { e.target.src = '/default-post-placeholder.png'; }}
                />
              ) : null;
            })()}
          </Link>
