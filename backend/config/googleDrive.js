...
class GoogleDriveManager {
  ...
  async deleteFile(fileId) {
    ...
    try {
      // Вместо окончательного удаления перемещаем в корзину
      ...
      console.log('[GOOGLE_DRIVE] File moved to trash:', fileId);
      return true;
    } catch (error) {
  ...
  /**
   * Удаляет thumbnail аватара по username
  ...
  async deleteAvatarThumbnail(username) {
    ...
    try {
      ...
      // Ищем thumbnail файлы с разными расширениями
      const possibleThumbnailNames = [
        `thumb_${safeUsername}.webp`,
      ...
      console.log(`[GOOGLE_DRIVE] Удалено thumbnail файлов: ${deletedCount}`);
      return deletedCount > 0;
    } catch (error) {
      console.error('[GOOGLE_DRIVE] Ошибка удаления thumbnail аватара:', error.message);
      return false;
    }
  }
